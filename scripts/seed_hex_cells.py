...
For each hex in `hex_data`, do a point-in-polygon lookup against the inserted
segments via SpatiaLite ST_Contains, then bulk-insert into hex_cells. Hexes
whose centroid is not inside any segment are dropped.

A SpatiaLite spatial index on `segments.geom` is created in migration 002,
so the per-hex query is ~O(log N) — fine for ~1k-5k fine hexes per mission.
"""
...
def seed_hex_cells(
    ...
    """Assign each fine hex to a segment via ST_Contains, then bulk-insert.

    Returns count of hex_cells inserted. Hexes outside every segment polygon
    are dropped (logged at info level).
    """
